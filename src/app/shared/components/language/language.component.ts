...
export class LanguageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.languageTitle = VOCABULARY.find(
      (v) => v.isoCode === `fr`
    ).sentences.languageTitle;
    this.nativeLanguageTitle = VOCABULARY.find(
      (v) => v.isoCode === `en`
    ).sentences.languageTitle;
    this.languagePlaceholderInput = VOCABULARY.find(
      (v) => v.isoCode === `fr`
    ).sentences.languagePlaceholderInput;
    this.translatedLanguagePlaceholder = VOCABULARY.find(
      (v) => v.isoCode === `en`
    ).sentences.translatedLanguagePlaceholder;
    this.languageCancelBtn = VOCABULARY.find(
      (v) => v.isoCode === `fr`
    ).sentences.languageCancelBtn;
    this.languageSubmitBtn = VOCABULARY.find(
      (v) => v.isoCode === `fr`
    ).sentences.languageSubmitBtn;
  }
  ...
  ngAfterContentChecked() {
    ...
    if (this.languageChoice !== undefined) {
      ...
      this.translatedLanguagePlaceholder =
        changeWording.sentences.translatedLanguagePlaceholder;
  ...
  private _filter(value): Vocabulary[] {
    ...
    return this.countries.filter((option) =>
      option.countryNameNative.toLowerCase().includes(value)
    );
  }
